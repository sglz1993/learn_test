package org.py.spring.mybatis.hello.doc;
 ...
 */
// 指名是spring的配置类，通过starter配置时，其实该注解可以不指定
@org.springframework.context.annotation.Configuration
// 只有当这两个类存在时该配置起作用
@ConditionalOnClass({ SqlSessionFactory.class, SqlSessionFactoryBean.class })
// 当只存在 DataSource Bean实例时起作用
@ConditionalOnSingleCandidate(DataSource.class)
// 使能 MybatisProperties 配置类
...
public class MybatisAutoConfiguration implements InitializingBean {
    ...
    // InitializingBean Bean初始化完成后check配置文件是否存在
    @Override
    ...
    public SqlSessionFactory sqlSessionFactory(DataSource dataSource) throws Exception {
        ...
        // 获取 SqlSessionFactoryBean 的所有属性名
    ...
    public SqlSessionTemplate sqlSessionTemplate(SqlSessionFactory sqlSessionFactory) {
        // https://vimsky.com/examples/detail/java-class-org.apache.ibatis.session.ExecutorType.html
    ...
    /**
     * 默认的Mapper扫描注册器，导入 Bean：AutoConfiguredMapperScannerRegistrar
     *      为了扫描Spring项目 BeanFactory 中的 @Mapper 注解创建Bean
     * 说明：
     *      @Import 注解的类不是必然被作为组件注册
     *
